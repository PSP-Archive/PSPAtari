...
typedef unsigned short bpp;

bpp *
psp_sdl_get_vram_addr(uint x, uint y)
{
  bpp *vram = (bpp *)back_surface->pixels;
...
{
  int x; int y;
  bpp *vram = psp_sdl_get_vram_addr(0,0);
...
{
  if (blit_surface) {
    bpp *vram = (bpp *)blit_surface->pixels;
...
{
  bpp *vram = (bpp *)psp_sdl_get_vram_addr(x, y);
  int xo, yo;
  if (mode == PSP_SDL_XOR) {
    for (xo = 0; xo < w; xo++) {
      vram[xo] ^=  border;
...
psp_sdl_load_png(SDL_Surface* my_surface, char* filename)
{
  ...
  for (y=0; y<height; y++) {
    ...
    for (x=0; x<width; x++) {
      ...
      bpp v = (((r >> 3) << systemRedShift  ) & systemRedMask) |
...
psp_sdl_save_thumb_png(SDL_Surface* my_surface, char* filename)
{
  ...
  int x;
  int y;
  bpp* src_pixel = (bpp*)blit_surface->pixels;
  bpp* dst_pixel = (bpp*)my_surface->pixels;
  bpp* scan_src_pixel = 0;
