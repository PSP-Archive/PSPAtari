...
psp_kbd_enter_irkeyb()
{
  ...
  if (psp_irkeyb != PSP_IRKEYB_EMPTY) {
    ...
    if (psp_irkeyb == 0x8) {
      atari_key = ATARIK_DELETE;
    } else
    if (psp_irkeyb == 0x9) {
      atari_key = ATARIK_TAB;
    } else
    if (psp_irkeyb == 0xd) {
      atari_key = ATARIK_RETURN;
    } else
    if (psp_irkeyb == 0x1b) {
      atari_key = ATARIK_ESC;
    } else
    if (psp_irkeyb == PSP_IRKEYB_UP) {
      atari_key = ATARIK_UP;
    } else
    if (psp_irkeyb == PSP_IRKEYB_DOWN) {
      atari_key = ATARIK_DOWN;
    } else
    if (psp_irkeyb == PSP_IRKEYB_LEFT) {
      atari_key = ATARIK_LEFT;
    } else
    if (psp_irkeyb == PSP_IRKEYB_RIGHT) {
      atari_key = ATARIK_RIGHT;
    } else {
...
# endif
  ...
  if (ATARI.atari_auto_fire) {
    ...
    if ((delta_stamp < 0) || 
      ...
      atari_key_event(ATARIK_JOY_FIRE1, ATARI.atari_auto_fire_pressed);
